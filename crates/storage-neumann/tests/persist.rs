...
#[tokio::test]
async fn try_new_fails_fast_on_bad_sled_path() {
    // Point data_path at a file (not a directory) — sled should reject it.
    let dir = tempdir().expect("tempdir");
    let file_path = dir.path().join("not_a_dir.txt");
    std::fs::write(&file_path, b"oops").expect("write");

    let config = NeumannConfig {
        endpoint: "http://localhost:7777".to_string(),
        namespace: "fail-test".to_string(),
        data_path: Some(file_path),
    };

    let result = NeumannStore::try_new(config);
    assert!(
        result.is_err(),
        "try_new should return Err when sled cannot open the path"
    );
}
